...
const NavBar = () => {
  ...
  const changeRole = async () => {
    try {
      const { data } = await axios.post("/api/user/change-role");
      if (data.success) {
        setIsOwner(true);
  ...
  return (
    ...
    xl:px-32 py-4 text-gray-600 border-b border-borderColor sticky top-0 z-[100] transition-all ${location.pathname === "/" ? "bg-light" : "bg-white"}`}
    >
      <Link to="/">
        <motion.img
